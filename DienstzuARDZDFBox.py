    ...
    def extract_info(table):
        ...
        if table:
            for row in table.find_all('tr'):
                ...
                if len(cols) >= 5:
                    info.append({
                        'dienstname': cols[0].text.strip(),
                        'dienstzeit': cols[1].text.strip(),
                        'arbeitsplatz': cols[2].text.strip(),
                        'pausenzeit': cols[3].text.strip(),
                        'task': cols[4].text.strip()
                    })
        return info
